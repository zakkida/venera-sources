class ExFangliding extends ComicSource {
    // Note: The fields which are marked as [Optional] should be removed if not used

    // name of the source
    name = "ExHentai (fangliding 镜像)"
    ...
    async checkEHEvent() {
        ...
        const res = await Network.get(this.baseUrl + "/news.php", {});
        if (res.status !== 200) {
            return;
        }
    ...
    account = {
        ...
        loginWithWebview: {
            ...
            onLoginSuccess: async () => {
                ...
                cookies.forEach((cookie) => {
                    cookie.domain = ".ex.fangliding.eu.org"
                })
                Network.setCookies("https://ex.fangliding.eu.org", cookies)
            },
        },
        ...
        loginWithCookies: {
            ...
            validate: async (values) => {
                ...
                Network.deleteCookies('https://ex.fangliding.eu.org')
                Network.setCookies('https://ex.fangliding.eu.org', cookies)
    ...
    get baseUrl() {
        return 'https://ex.fangliding.eu.org';
    }
    ...
    async onLoadFailed() {
        let cookies = await Network.getCookies('https://ex.fangliding.eu.org')
        cookies.forEach((c) => {
    ...
    categoryComics = {
        ranking: {
            ...
            load: async (option, page) => {
                let res = await this.getGalleries(`${this.baseUrl}/toplist.php?tl=${option}&p=${page-1}`, true);
    ...
    comic = {
        ...
        archive: {
            getArchives: async (cid) => {
                ...
                let index = this.baseUrl.includes("exhentai") || this.baseUrl.includes("fangliding") ? 1 : 3
