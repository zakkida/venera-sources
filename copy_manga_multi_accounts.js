class CopyManga extends ComicSource {
    ...
    get headers() {
        return this._headersForToken(this.loadData("account_token_0"));
    }
    ...
    static defaultApiUrl = 'api.copy202601.com'

    // 网页端搜索接口 (/api/kb/web/searchb/comics) 已废弃返回 404,
    // 默认使用基础 API; 若 www.copy20.com 可用仍会被 refreshSearchApi 覆盖
    ...
    favorites = {
        ...
        loadFolders: async (comicId) => {
            ...
            let favorited = [];
            if (comicId) {
                favorited = await this._checkFavoriteAccounts(comicId);
            }
        ...
        addOrDelFavorite: async (comicId, folderId, isAdding) => {
            let token = this.loadData(`account_token_${folderId}`);
    ...
    async _loadSingleFavorites(token, page) {
        ...
        return {
            comics: result.list.map(parseComic),
            maxPage: Math.ceil(result.total / 30)
        };
    }
